...
uint8_t m_usart2_rx_buffer[USART3_RX_RING_BUFFER_SIZE];
...
void MX_USART2_UART_Init(void)
{
  ...
  */
  ...
  USART_InitStruct.BaudRate = m_last_usart3_baud;
  USART_InitStruct.DataWidth = LL_USART_DATAWIDTH_8B;
  USART_InitStruct.StopBits = LL_USART_STOPBITS_1;
  USART_InitStruct.Parity = LL_USART_PARITY_NONE;
  USART_InitStruct.TransferDirection = LL_USART_DIRECTION_TX_RX;
...
//                DEBUG_RAW("%.*s", pos - m_old_usart3_dma_rx_pos, &m_usart3_rx_dma_buffer[m_old_usart3_dma_rx_pos]);
				...
				if (bytes_write != lwrb_write(&m_ringbuffer_usart2_rx, &m_usart3_rx_dma_buffer[m_old_usart3_dma_rx_pos], bytes_write))
                {
                    lwrb_reset(&m_ringbuffer_usart2_rx);
            ...
            else
            {
                ...
                if (pos > 0)
                {
                    if (pos != lwrb_write(&m_ringbuffer_usart2_rx, &m_usart3_rx_dma_buffer[0], pos))
                    {
                        lwrb_reset(&m_ringbuffer_usart2_rx);
                        DEBUG_ISR("Ringbuffer full\r\n");
                    }
